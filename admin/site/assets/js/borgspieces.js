...
(function($)
{
		...
		$.fn.tinies = {
			...
			init:function(){
								console.log("speed"+config.cubespeed);
           ...
           interval = setInterval(function(){
			...
			primaryShape:function()
			{
				...
				for( i=0; i< parseInt(config.rows) * parseInt(config.cols) ; i++)
				{
					$('#surface'+surfacecounter+' .cimagesrotator').children('div').eq(i).stop(true, true).animate({position:'absolute', left:lefts[ls]+'px', top:tops[ts]+'px' }, parseInt(config.imagebuildspeed), function(){
													//console.log("tiny");
