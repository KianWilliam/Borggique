...
function BorggiqueBuildRoute(&$query)
{
		$segments = array();
		
			$app		= JFactory::getApplication();
	        $menu		= $app->getMenu();
	        $params		= JComponentHelper::getParams('com_borggique');
	...
	if (isset($query['view']) && ($mView == $query['view']) and (isset($query['title'])) and ($mId == intval($query['title']))) {
		...
		if (isset($view) and  $view == 'borggique' ) {
...
function BorggiqueParseRoute($segments)
{
		$vars = array();
